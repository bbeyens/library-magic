...
- Generate endpoint: `openai/gpt-image-2`
- Edit endpoint: `openai/gpt-image-2/edit`
- Good for: higher-fidelity OpenAI-family generation and editing with flexible custom sizes
- Typical controls surfaced by fal:
  - `prompt`
  - `num_images`
  - `image_size`
  ...
  - edit uses `image_urls`
- Workflow note from this repo:
  - use custom `{width,height}` image sizes when matching a tracked OpenAI run
  - keep reference count low for identity-critical sprite work
  - treat it as the closest fal-side comparison to direct `gpt-image-2` runs
