---
name: auto
description: Run the full end-to-end feature workflow in one go: autogoal, grill-with-docs, to-prd, to-issues, implement, game/browser playtest, and review. Use when the user asks for automatic feature execution, full workflow, e2e delivery, or says to run auto.
...
After the grill phase, continue in one-shot execution. Do not ask for plan approval, issue approval, or implementation approval.

## Workflow

1. **Start with `autogoal`.**
   - Create or continue one measurable goal for the feature.
   ...
   - If the work is truly micro, say so and downgrade to the smallest useful path. If the user explicitly invoked `auto`, still keep a lightweight goal.

2. **Run `grill-with-docs`.**
   - Read the repo docs and relevant code.
   - Ask only the questions that materially change the feature.
   - Keep the grill short. The output should be decisions, not a meeting transcript.
...
```text
autogoal -> grill-with-docs -> to-prd -> to-issues -> implement first slice -> browser/game playtest -> review
```
