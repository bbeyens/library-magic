...
description: "Create high-quality Codex skills (philosophy-first, anti-patterns, variation); use when designing or improving `.codex/skills/*` skills."
metadata:
  short-description: "Philosophy-first skill design for Codex."
---

# Skill Creator Plus

An advanced framework for creating skills that **unlock Codex's capabilities** rather than constraining them to templates.

## Codex Mechanics (Read First)

Skills in Codex use **progressive disclosure**:
- At startup, Codex loads only a skill’s `name`, `description`, and the `SKILL.md` file path.
- A skill can be invoked **explicitly** (type `$skill-name` or use `/skills`) or **implicitly** when the task matches its description.
- When invoked, Codex can read the full `SKILL.md` plus any bundled files under `references/`, `scripts/`, and `assets/`.

**Frontmatter constraints (validated by `scripts/quick_validate.py`)**:
- `name`: required; hyphen-case; ≤64 chars; no leading/trailing hyphen; no consecutive hyphens
- `description`: required string; ≤1024 chars; use an inline/quoted value (not YAML folded/literal scalars like `>` or `|`); avoid `<` and `>`; make it trigger-rich (keywords, file types, use-cases)
- Optional: `metadata.short-description` (user-facing label)
- Optional (validator-supported): `license`, `allowed-tools`

**Placement & precedence**:
- Codex loads skills from multiple locations/scopes. Higher-precedence locations override lower-precedence skills with the same `name`.
- Prefer repo-scoped skills under `.codex/skills/` when they should travel with the project; use `~/.codex/skills/` for personal skills.
- Prefer real skill directories over symlinks unless you know your runner follows them.
...
- **Purpose**: What procedural knowledge will this skill provide that Codex doesn't already have?
- **Philosophy**: What mental framework should guide decisions in this domain?
- **Triggers**: When should this skill be selected? What specific use cases, file types, and keywords should appear in the `description`?
...
- Documentation Codex should reference when invoked
...
---
name: skill-name
description: "Comprehensive description of what, when, and specific triggers; include use cases and file types that should match."
metadata:
...
### 7. Ship the Skill (Codex)

Codex uses **folders**, not `.skill` packages:

1. Put the skill folder in a valid location (repo: `.codex/skills/<name>/`, user: `~/.codex/skills/<name>/`, etc.)
2. Restart Codex (or reload skills) and confirm it appears in `/skills`.
3. Test both:
   - Explicit invocation: `$<skill-name>` + a realistic task
   - Implicit invocation: a prompt that should match the `description`
...
4. Re-analyze and re-test triggering
...
Codex uses progressive disclosure: keep `SKILL.md` lean and move deep dives into `references/`.
...
| Aspect | `$skill-creator` (built-in) | skill-creator-plus |
|--------|---------------|-------------------|
| Approach | Bootstraps structure | Philosophy-first + quality heuristics |
...
- Empower Codex to do extraordinary work

This skill exemplifies these principles. Study its structure, use the tools, and create skills that unlock capabilities.

**Codex is capable of extraordinary work in skill creation. These guidelines illuminate the path—they don't fence it.**
