#!/usr/bin/env python3
"""
Minimal Google Gemini image-edit / multi-image-reference wrapper.

Sends one or more reference images plus a text edit prompt to:
  POST /v1beta/models/{model}:generateContent
with responseModalities=["TEXT","IMAGE"], and writes any inline-image parts to disk.

Usage:
  GEMINI_API_KEY=... python3 gemini_image_edit.py \
    --image refs/identity.png \
    --image refs/layout.png \
    --prompt "Use image 1 for identity, image 2 for composition. Change background only." \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Edit / refer with Google Gemini image models.")
    parser.add_argument(
        "--image",
        type=Path,
        action="append",
        required=True,
        help="Reference image. Repeat --image to send multiple references.",
    )
    parser.add_argument("--prompt", required=True, help="Edit / composition instruction.")
    parser.add_argument(
        "--model",
    ...
    parser.add_argument("--aspect-ratio", default=None, help='e.g. "16:9", "1:1".')
    parser.add_argument("--image-size", default=None, help='"512", "0.5K", "1K", "2K", "4K".')
    parser.add_argument(
        "--thinking-level",
        default=None,
        choices=sorted(VALID_THINKING),
        help='"minimal" or "High".',
    )
    parser.add_argument(
        "--include-thoughts",
        action="store_true",
        help="Include thought signatures in the response.",
    )
    parser.add_argument(
        "--google-search",
        action="store_true",
...
def validate(args: argparse.Namespace) -> None:
    ...
    limit = REFERENCE_LIMITS[args.model]
    if len(args.image) > limit:
        raise SystemExit(
            f"{args.model} supports up to {limit} reference images; got {len(args.image)}"
        )
...
def mime_for_path(path: Path) -> str:
    ...
    raise SystemExit(f"Unrecognized image mime type for {path}")
