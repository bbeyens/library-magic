...
Quick review page for the `s` facing `idle` animation. Mode: `image`. Frames: `10`. FPS: `6`.
...
First `10` implied grid cells cropped from the pose board for review only. These are not the runtime source of truth. Generation grid: `4x3`. Cell size: `384x384`. Review image size: `1688x204`.
...
Foreground components recovered from the full pose board and ordered by the declared grid. These recovered components are the runtime source. Review image size: `1011x204`.
...
Recovered components placed on a shared padded native review canvas without scaling. This is the first animation review checkpoint before pixel snap or 256x256 runtime normalization. Review image size: `1458x204`.
...
Before/after comparison showing why implied grid crops are review artifacts only. Review image size: `1288x352`.
...
Before/after comparison showing variable-size recovered components padded to a shared native review canvas without scaling. Review image size: `1118x352`.
...
Before/after comparison showing recovered components against raw variable-size pixel snap outputs. Review image size: `791x352`.
...
Before/after comparison for cleanup output to shared-scale layout input. Review image size: `787x352`.
...
Before/after comparison for final placement into runtime cells. Review image size: `1288x352`.
