...
Quick review page for the `s` facing `idle` animation. Mode: `image`. Frames: `8`. FPS: `6`.
...
Foreground components recovered from the full pose board and ordered by the declared grid. These recovered components are the runtime source. Review image size: `590x204`.
...
Recovered components placed on a shared padded native review canvas without scaling. This is the first animation review checkpoint before pixel snap or 256x256 runtime normalization. Review image size: `1240x204`.
...
Recovered components looped on the padded native review canvas before runtime normalization. GIF canvas: `512x560`.
...
Before/after comparison showing variable-size recovered components padded to a shared native review canvas without scaling. Review image size: `952x352`.
...
Before/after comparison for cleanup output to shared-scale layout input. Review image size: `463x352`.
